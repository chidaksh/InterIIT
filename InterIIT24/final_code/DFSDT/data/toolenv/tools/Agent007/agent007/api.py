...
def summarize_objects(objects = None):
    ...
    try: 
        ...
        for obj in objects:
            if type(obj) == str:
                obj = next((item for item in work_items_db if item.id == obj), None)
            summarized.append(obj.summarize())
    except:
        return {"error": """Not a valid list of objects/ object ids. Example: "['WK-001', 'WK-002']"""""}    
    return summarized
    

def prioritize_work_items(objects = None):
...
def get_similar_work_items(work_id = None):
    ...
    if work_id is None:
        ...
        similar_items_ids = [item.id for item in similar_items]
